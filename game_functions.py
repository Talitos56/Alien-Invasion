...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb,
    ...
    collisions = pygame.sprite.groupcollide(bullets, aliens,
                                            True, True)
    if collisions:  # devolve um dicionário
        explosion_sound()
        for aliens in collisions.values():
            # O valor associado a cada projétil é uma
            # lista de alienígenas
...
def start_new_level(ai_settings, screen, stats, sb,
                    ship, aliens, bullets):
    """Inicia um novo nível quando a frota é destruída
...
def get_number_aliens_x(ai_settings, alien_width):
    """Determina o número de alienígenas que cabem numa linha."""
...
def check_fleet_edges(ai_settings, aliens):
    ...
    for alien in aliens.sprites():
        if alien.check_edges():  # se retornar True no módulo alien
...
def check_aliens_bottom(ai_settings, stats, sb, screen,
                        ship, aliens, bullets):
    """Verifica se algum alienígena alcançou a parte
        inferior da tela."""
...
def save_high_score(stats):
    """Salva a pontuação máxima num arquivo
...
def play_music():
    """Toca a música de fundo quando abrir o jogo"""
...
def stop_music():
    """Para de tocar a música de fundo"""

    pygame.mixer.music.stop()


def explosion_sound():
    """Toca o som de explosão quando atinge um alien"""
